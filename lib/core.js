...
module.exports = {
	...
	// Get a name that matches a device's class name based off a device's port names
	...
	requireMethods(methods, supports, path) {
		for (const support of supports) {
			const method = this.requireMethod(`${path}${support}.js`);
			if (method) {
				methods.push(method);
			}
		}
	...
	makeProperties(object) {
		const props = object.__properties__;
		if (props) {
	...
	addShared(object, methods) {
		// Shared instace or class methods
		Object.assign(
			// Instance / class
			object,
			// Methods
