...
const getSimilarBytes = function(message, template) {
	...
	const captures = [];
	// Use the template as the "state" to be checking against
	let templateIndex = 0;
	let isCapturing = false;

	// Loop over the messages as the "input" while matching against the current state
	for (let messageIndex = 0; messageIndex < message.length;) {
		const templateByte = template[templateIndex];
		const nextTemplateByte = template[templateIndex + 1];
		const messageByte = message[messageIndex];

		switch (templateByte) {
			// Match exact byte
			case messageByte: {
				// Matched! Next state and message byte
				templateIndex++;
				messageIndex++;
				break;
			}
			...
			case "*": {
				...
				// If the next state could match it, stop capturing and retry
				if (typeof nextTemplateByte === "string" || nextTemplateByte === messageByte) {
					// Next state
					templateIndex++;
					// Stop capturing
					isCapturing = false;
					// Don't increase the messageIndex and continue to retry with the same message byte
					continue;
				}

				// Capture this message byte
				_.last(captures).push(messageByte);
			...
			// No match
			default: {
				// This input didn't match, stop trying to match by returning false
				return false;
			}
		}
	}
	...
	for (; templateIndex < template.length; templateIndex++) {
		...
		switch (templateByte) {
			// Add undefined to the captures
			case "?": {
				captures.push(undefined);

				break;
			}
			// Add an empty array to the captures
			case "*": {
				captures.push([]);
				break;
			}
			// Needed to match more than what was in the input but didn't
			default: {
				return false;
			}
		}
