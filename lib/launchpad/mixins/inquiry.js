...
module.exports = function() {
	...
	methods(
		...
		// Instance
		{
			...
			// Sends device inquiry for inquireDevice or for use with own event listener
			sendDeviceInquiry(deviceId = this.deviceId) {
			...
			inquireDevice() {
				return this.inquire("device", ...arguments);
			},
			// Sends version inquiry for inquireVersion or for use with own event listener
			sendVersionInquiry() {
				// Send with default manufacturer ID using undefined and empty the model ID using []
			...
			inquireVersion() {
				return this.inquire("version", ...arguments);
			},
			inquire(event, ...args) {
				return new Promise((resolve, reject) => {
					// Send
					try {
						this["send" + _.startCase(event) + "Inquiry"](...args);
					} catch (error) {
						reject(error);
					}
