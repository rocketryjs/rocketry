...
const light = function(color) {
	...
	if (color.type === "rgb") {
		...
		return this.device.send[status]([this.note, color.value], 1); // TODO
	}
};
light.stop = function() {
...
const flash = function(color) {
	...
	if (Array.isArray(color)) {
		...
		// Basic
		this.device.send.sysex([this.device.constructor.sysex.prefix, 35, 0, this.note, color.value]);
	}
...
const pulse = function(color) {
	...
	if (Array.isArray(color)) {
		...
		// Basic
		this.device.send.sysex([this.device.constructor.sysex.prefix, 40, 0, this.note, color.value]);
	}
