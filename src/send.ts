...
*/
export const send = function<T extends Device<T>> (this: T, message: Message): T {
...
send.noteOff = function<T extends Device<T>> (this: T, message: Message, channel: Channel = 1) {
	return this.send(addStatusByte(message, 128, channel));
};
// Status bytes from 144 to 159
send.noteOn = function<T extends Device<T>> (this: T, message: Message, channel: Channel = 1) {
	return this.send(addStatusByte(message, 144, channel));
};
// Status bytes from 160 to 175
send.polyKeyPressure = function<T extends Device<T>> (this: T, message: Message, channel: Channel = 1) {
	return this.send(addStatusByte(message, 160, channel));
};
// Status bytes from 176 to 191
send.controlChange = function<T extends Device<T>> (this: T, message: Message, channel: Channel = 1) {
	return this.send(addStatusByte(message, 176, channel));
};
// Status bytes from 192 to 207
send.programChange = function<T extends Device<T>> (this: T, message: Message, channel: Channel = 1) {
	return this.send(addStatusByte(message, 192, channel));
};
// Status bytes from 208 to 223
send.monoKeyPressure = function<T extends Device<T>> (this: T, message: Message, channel: Channel = 1) {
	return this.send(addStatusByte(message, 208, channel));
};
send.channelPressure = send.monoKeyPressure;
// Status bytes from 224 to 239
send.pitchBend = function<T extends Device<T>> (this: T, message: Message, channel: Channel = 1) {
	return this.send(addStatusByte(message, 224, channel));
};
// Send `[sysex status byte, IDs, message, EOX]`
send.systemExclusive = function<T extends Device<T>> (this: T, message: Message) {
	return this.send([240, ...message, 247]);
};
send.sysEx = send.systemExclusive;
send.sysex = send.systemExclusive;

export const makeSend = function <T extends Device<T> | void> (device: T): Send<void, T> {
	return bindDeep(send as Send<T>, device);
};
